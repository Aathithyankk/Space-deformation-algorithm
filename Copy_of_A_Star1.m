...
x_val = 1;
y_val = 1;
axis([1 MAX_X+1 1 MAX_Y+1]) %Creates the axes
grid on;
hold on;
...
while (but ~= 1) %Repeat until the Left button is not clicked
    [xval,yval,but]=ginput(1); %gets 1 point from the current axis and returns the x and y returns 1 if left mouse button is pressed
end
xval=floor(xval); %Approx
yval=floor(yval); %Approx
...
while but == 1
    ...
    x_box = [xval x1 x1 xval];
    y_box = [yval yval y1 y1];
    fill(x_box,y_box,'k')
...
obstacles = CLOSED;
CLOSED_COUNT=size(CLOSED,1); %returns number of rows in CLOSED
...
if ( (xval == xTarget) && (yval == yTarget))
 ...
 for i=j:-1:1
  pause(.25);
  set(p,'XData',Optimal_path(i,1)+.5,'YData',Optimal_path(i,2)+.5); % to move the blue color circle after finding the optimal path
 drawnow ;
 end;
